...
function Grid() {
    ...
    _.range(0,7).forEach(function(num) {
        slots.push(new Slot(num, 1));
    });

    _.range(7,19).forEach(function(num) {
        slots.push(new Slot(num, 2));
    });

    _.range(19,37).forEach(function(num) {
        slots.push(new Slot(num, 3));
    });
...
Grid.prototype.getDPS = function(withEffects){
    var sum = 0;
    this.slots.forEach(function(slot) {
        if(slot.hexaEntity.hexa instanceof DpsHexa) {
...
Grid.prototype.getSlot = function(id) {
    return this.slots.filter(function(slot) {
        return slot.id == id;
    })[0];
}

Grid.prototype.getSlotByPos = function(position) {
    return this.getSlot(this.positionMap[position.q][position.r]);
}

Grid.prototype.emptySlotCount = function() {
    return this.slots.filter(function(slot){
        return slot.tier <= this.tier && slot.hexaEntity == undefined;
    }).length;
}

Grid.prototype.maxUpgrades = function() {
    switch (this.tier) {
        case 1:
            return 1;
        case 2:
